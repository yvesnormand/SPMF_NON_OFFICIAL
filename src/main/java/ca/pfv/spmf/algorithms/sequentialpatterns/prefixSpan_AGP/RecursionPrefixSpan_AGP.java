...
class RecursionPrefixSpan_AGP {
     ...
     */
    public RecursionPrefixSpan_AGP(AbstractionCreator abstractionCreator, Saver saver, long minSupportAbsolute,
                                   PseudoSequenceDatabase pseudoDatabase, Map<Item, BitSet> mapSequenceID) {
    ...
    public void execute(boolean keepPatterns, boolean verbose) {
        ...
        for (Item item : keySetList) {
            ...
            PseudoSequenceDatabase projectedContext = makePseudoProjections(item, pseudoDatabase, abstractionCreator.CreateDefaultAbstraction(),
                    true);
     ...
     */
    private PseudoSequenceDatabase makePseudoProjections(Item item, PseudoSequenceDatabase database, Abstraction_Generic abstraction,
                                                         boolean firstTime) {
        ...
        for (Pair pair : pairs) {
            ...
            if (pair.getSupport() >= minSupportAbsolute) {
                ...
                if (projection != null && projection.size() >= minSupportAbsolute) {
                    ...
                    prefixSpanLoop(newPrefix, k + 1, projection, keepPatterns, verbose); // récursion
                }
            }
        }
    }
