...
public class CandidateInSequenceFinder {
    ...
    public void isCandidatePresentInTheSequence_qualitative(Pattern candidate, Sequence sequence, int k, int length, List<int[]> position) {
        ...
        while (position.get(length)[0] < sequence.size()) {
            ...
            if (length == 0) {
                 ...
                 */
                pos = creator.findPositionOfItemInSequence(sequence, itemPair, abstractionPair, previousAbstraction, position.get(length)[0],
                        position.get(length)[1], position.get(length - 1)[0], position.get(length - 1)[1]);
            }
     ...
     * If the itemset is exceeded, we change to the next itemset, establishing the item index in 0 and so, in this case, our returning value is <
     * currentItemsetIndex +1 , 0 >.
     *
