...
//			int seqCount =0;
            ...
            for (int sequenceID : sidset) {
                ...
                if (period != null) {
                    ...
                    boolean hasBackwardExtension = findAllFrequentPairsForBackwardExtensionCheck(alreadyVisitedSID.size(), prefix, period, i,
                            mapPaires, itemI, itemIm1);
...
//					boolean hasExtension = findAllFrequentPairsForBackwardExtensionCheck(alreadyVisitedSID.size(), prefix, period, i, mapPaires,
//					itemI, itemIm1);
//					if(hasExtension) {
//						return true;
//					}
...
//		long start = System.currentTimeMillis();
        ...
        return pair.getSequenceIDs().size() >= minsuppAbsolute;
...
//		debugAddPairTime += System.currentTimeMillis() - start;
    ...
    private Map<Integer, Set<Integer>> findSequencesContainingItems(SequenceDatabase database) {
        ...
        Map<Integer, Set<Integer>> mapSequenceID = new HashMap<Integer, Set<Integer>>(); // pour conserver les ID des séquences: <Id Item, Set d'id
        // de séquences>
        // for each sequence
        for (Sequence sequence : database.getSequences()) {
     ...
     */
    private List<PseudoSequenceBIDE> buildProjectedContextSingleItem(Integer item, Map<Integer, PseudoSequenceBIDE> initialDatabase2,
...
//					}
    ...
    public void printStatistics(int size) {
        final String r = "=============  Algorithm MaxSP - STATISTICS =============\n Total time ~ " +
                         (endTime - startTime) +
                         " ms\n" +
                         " Maximal sequential pattern count : " +
                         patternCount +
                         '\n' +
                         " Max memory (mb):" +
