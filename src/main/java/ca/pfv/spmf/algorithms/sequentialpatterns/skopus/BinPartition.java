...
public class BinPartition {
    private final ItemsetRec irLeft;
    private final ItemsetRec irRight;
    ...
    public boolean equals(Object obj) {
        return (irLeft.equals(((BinPartition) obj).getLeft()))
    ...
    private boolean lessThan(ItemsetRec isParaLeft, ItemsetRec isParaRight) {
        ...
        if (isParaLeft.size() < isParaRight.size()) {
            ...
            for (int i = 0; i < isParaLeft.size(); i++) {
                int nl = isParaLeft.get(i);
                int nr = isParaRight.get(i);
                if (nl < nr) {
                    return true;
                } else if (nl > nr) {
                    return false;
