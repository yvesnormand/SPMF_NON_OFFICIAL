...
//				// Initialize the IE lists and SE lists for that sequence
        ...
        for (int i = 0; i < frequentItems.size(); i++) {
            ...
            if (item1Border.size() >= minsup) {
                ...
                genPatterns(prefix, item1Border, frequentItems, frequentItems, item1, true);  // true, to disallow I-extension because we explore
                // 2-IE sequences separately
            }
    ...
    private void genPatterns(Prefix prefix, List<Position> prefixBorder, List<Integer> sn, List<Integer> in, int hasToBeGreaterThanForIStep,
...
//				Integer lastItem = lastItemset.get(lastItemset.size()-1);
            ...
            if (support >= minsup) {
                ...
                List<Position> newBorder = recalculateBorderForIExtension(lastItemset, prefixBorder, hasToBeGreaterThanForIStep, item,
                        willAddSecondItem);
    ...
    public void printStatistics() {
        final String r = "=============  LAPIN - STATISTICS =============\n Total time ~ " +
                         (endTime - startTime) +
                         " ms\n" +
                         " Frequent sequences count : " + patternCount +
                         '\n' +
                         " Max memory (mb) : " +
