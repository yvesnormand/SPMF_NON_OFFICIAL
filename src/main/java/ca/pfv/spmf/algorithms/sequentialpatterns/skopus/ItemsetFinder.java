...
public class ItemsetFinder {
    ...
    private final int nTemplateLength = 10;    //ĬééΪ16
    ...
    private final ArrayList<Double> alMOuterBaseSupport; // the base support in all the partitions
    private final HashMap<ItemsetRec, Integer> hmMCheckedSequenceCount;
    private final BinPartitionTemplate[] aryMBinPartitionTemplate;
    private final CombinTemplate[][] aryMCombineTemplate;
...
//		aryMCombineTemplate = new CombinTemplate[GlobalData.nSampleMaxLength + 1][(int) (GlobalData.nSampleMaxLength / 2) + 2];
        ...
        aryMBinPartitionTemplate = new BinPartitionTemplate[nTemplateLength + 1]; //éééé11éڵĲéééééééģé壬éééé10éģéʵʱéééé
        aryMCombineTemplate = new CombinTemplate[nTemplateLength + 1][(nTemplateLength / 2) + 1];        //éééé11éڵĲéééééééģé壬éééé10éģéʵʱéééé
...
//		System.out.println("list to compute the mean of: "+al.toString());
    ...
    //20150121ééʹééGeoffééééȷ˼·ééд
    // éӱéѡitemsetséУéééééééééȤéļééé
    public void generateResultItemsets() {
        // 1 ééʼéésequence pattern setéésingle itemééééavailable item sets
...
//		System.out.println("Bootstrap done");
    ...
    private void skopus(final ItemsetRec irCandidateSequence,
        ...
        // 20150211-Ϊé˼ӿéʵéééٶȣééééгéééé޶éééGlobalData.nMaxResultPatternLengthéééé
        ...
        // 2 ѭéééééAvailable Item setéé update new available item set and candidate
        for (int ixSymbol = 0; ixSymbol < iqAvailItems.size(); ixSymbol++) {
            ...
            //2.3ééééupper boundééééééééitem set
    ...
    private void checkSequenceLeverage(ItemsetRec isTheOneSequence) {
        ...
        // 2.ééééȫéééééܵébinary partition
        ...
        // 4.éémin supportéééémaxéééõébase support
        double nBaseCount = getMaxDouble(alMOuterBaseSupport);

        // 5.ééééInterestness value
    ...
    private void checkOnePartition(BinPartition bpOnePartition,
        ...
        for (Iterator<ItemsetRec> iter = cs.iterator(); iter.hasNext(); ) {
            // 3.1 ééϳéһéééµésequence
            ItemsetRec isNew = iter.next();

            // 3.2 é鿴éµésequenceééӦéééǵéǰéééé
...
//				+ "\n";
    ...
    // éééһééééééInterestingnesséééééésequence
    private void insertOneResultSequence(ItemsetRec ir) {
        ...
        if (pqMItemsetTopk.size() == GlobalData.nK) {
            ...
            if (newMin > dMMinValueGlobal) {
                dMMinValueGlobal = newMin; // ééééMPMééСֵééȫéֱééééééééڴ˴éééé¡é
            }
        }
...
// éééڽéééӴéСééé򣬸éééitemsetRecééééȤéévalueֵ
...
// éééڽééPriorityQueueééééıȽéé࣬СééééȤééééֵéڶéջéééééééڵééé?
