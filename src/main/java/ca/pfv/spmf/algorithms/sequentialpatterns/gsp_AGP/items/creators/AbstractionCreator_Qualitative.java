...
public class AbstractionCreator_Qualitative implements AbstractionCreator {
    ...
    public Pattern getSubpattern(Pattern extension, int index) {
        ...
        for (int i = 0; i < extension.size(); i++) {
            ...
            if (i != index) {
                ...
                if (i == nextIndex) {
                    //If we do no have any value in "abstraction" it means that it was not prepared before. In that case we take the abstraction
                    // appearing in the ith-element
                    if (abstraction == null) {
    ...
    public Abstraction_Generic createAbstraction(long currentTime, long previousTime) {
        boolean inTheSameItemset = currentTime == previousTime;
     ...
     */
    public int[] findPositionOfItemInSequence(Sequence sequence, Item itemPair, Abstraction_Generic absPair, Abstraction_Generic previousAbs,
                                              int itemsetIndex, int itemIndex, int previousItemsetIndex, int previousItemIndex) {
