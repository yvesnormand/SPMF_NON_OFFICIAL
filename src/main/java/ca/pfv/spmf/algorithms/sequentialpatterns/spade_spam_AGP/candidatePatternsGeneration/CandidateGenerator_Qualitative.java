...
public class CandidateGenerator_Qualitative implements CandidateGenerator {
     ...
     */
    @Override
    public List<Pattern> generateCandidates(Pattern pattern1, Pattern pattern2, int minSupport, boolean doNotExploreXY, boolean doNotExploreYX,
                                            boolean doNotExploreX_Y, boolean doNotExploreY_X) {
            ...
            if (!abstractionOfLastPairOfPattern1.hasEqualRelation() && !abstractionOfLastPairOfPattern2.hasEqualRelation()) {
                ...
                if (!lastPairOfPattern1.getItem().equals(lastPairOfPattern2.getItem())) {
                    ...
                    if (pattern1.compareTo(pattern2) < 0 && !doNotExploreXY) {
                        ...
                        newCandidate_equalRelation.add(pairCreator.getItemAbstractionPair(lastPairOfPattern2.getItem(),
                                qualitativeCreator.createAbstraction(true)));
                    } else if (!doNotExploreYX) {
                        newCandidate_equalRelation = pattern2.clonePattern();
                        newCandidate_equalRelation.add(pairCreator.getItemAbstractionPair(lastPairOfPattern1.getItem(),
                                qualitativeCreator.createAbstraction(true)));
                    }
