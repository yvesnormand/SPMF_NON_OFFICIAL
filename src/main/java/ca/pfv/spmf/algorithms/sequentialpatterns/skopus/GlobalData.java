...
public class GlobalData {
    ...
    public static boolean bSmoothedValue = false; // true使用Smoothed value作为support或的计算方法，即分子分母均加1é?
    public static int nInterestingnessMeasure = 1; // the flag of interestingness measure. 1: support(default), 2: leverage
    public static int nK = 100; //the K number of top K, the default value is 100é?
    public static int nMaxResultPatternLength = 0; // the maximum sequential pattern length. if the value <=0, there is no limit; otherwise, the
    public static double dSmoothCoefficient = 0.5;
    // result pattern length is less than nMaxLength.
