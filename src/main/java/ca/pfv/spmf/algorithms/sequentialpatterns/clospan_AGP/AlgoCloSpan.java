...
public class AlgoCloSpan {
    ...
    protected void cloSpan(SequenceDatabase database, boolean keepPatterns, boolean verbose, boolean findClosedPatterns,
        ...
        RecursionCloSpan algorithm = new RecursionCloSpan(abstractionCreator, saver, (int) minSupAbsolute, pseudoDatabase, mapSequenceID,
        ...
        if (findClosedPatterns) {
            ...
            numberOfFrequentPatterns = algorithm.numberOfFrequentPatterns();
            if (verbose) {
                System.out.println("CLOSPAN: The post-processing algorithm to remove the non-Closed patterns takes " + timeForPostProcessingStep +
                                   " seconds and finds " + numberOfFrequentPatterns + " Closed patterns");
            }
        } else {
            if (keepPatterns) {
                List<Pattern> outputPatternsFromMainMethod = algorithm.getFrequentPatterns();
