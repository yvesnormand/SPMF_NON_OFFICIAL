...
public class FrequentPatternEnumeration_ClaSP {
     ...
     */
    public FrequentPatternEnumeration_ClaSP(AbstractionCreator abstractionCreator, double minSupAbsolute, Saver saver, boolean findClosedPatterns,
    ...
    public void dfsPruning(Pattern patron, Trie trie, boolean verbose, Map<Integer, Map<Integer, Integer>> coocMapAfter, Map<Integer, Map<Integer,
            Integer>> coocMapEquals) {
        ...
        for (int i = 0; i < tam; i++) {
             ...
             */
            exploreChildren(new Pattern(eq.getPair()), eq, trie.getNodes(), trie.getNodes(), i + 1, coocMapAfter, coocMapEquals,
                    eq.getPair().getItem());
        }
    }

    private void exploreChildren(Pattern pattern, TrieNode currentNode, List<TrieNode> sequenceExtensions, List<TrieNode> itemsetsExtensions,
                                 int beginning, Map<Integer, Map<Integer, Integer>> coocMapAfter, Map<Integer, Map<Integer, Integer>> coocMapEquals
            , Item lastAppendedItem) {
...
//if (isAvoidable(pattern, currentTrie)) return; //Tin hide
        ...
        for (int k = beginning; k < itemsetsExtensions.size(); k++) {
            ...
            ItemAbstractionPair newPair = ItemAbstractionPairCreator.getInstance().getItemAbstractionPair(eq.getPair().getItem(),
...
//                currentTrie.mergeWithTrie(newTrieNode);
        ...
        for (int i = 0; i < itemsetExtensionSize; i++) {
             ...
             */
            if (isAvoidable) {
                new_sequenceExtension = firstSequenceExtensions;
            }
