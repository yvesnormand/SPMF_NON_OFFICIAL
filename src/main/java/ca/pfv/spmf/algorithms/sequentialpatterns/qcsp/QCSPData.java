...
public class QCSPData {
    // 102, 1003, 2003, 2010, 191919] etc
    private final int NULL_ITEM = 0; //for gaps
    private final int SEPERATOR_ITEM = -1;
    private final int END_ITEM = -2;
    //data
    ...
    private ListMap<Integer, Integer> itemPositions; //projection on first level, e.g. projection1.get(0) returns list of positions, that is [0,
        ...
        try {
            ...
            while (token != null) {
                ...
                if (item == NULL_ITEM) { //if item 0, add gap
                    ...
                    token = fs.nextToken();
                } else if (item == END_ITEM) { //if item -2, add max_size * alpha gaps, to avoid that quantile-based cohesive pattern occurrence
                    // spans multiple sequences
