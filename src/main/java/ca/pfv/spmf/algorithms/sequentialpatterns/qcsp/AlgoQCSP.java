...
public class AlgoQCSP {
    ...
    private static final Comparator<Pair<SequentialPattern, Double>> heapComparator = new Comparator<Pair<SequentialPattern, Double>>() {
    ...
    //state:
    private final List<Window> init = makeList(new Window(-1, -1, -1));
    ...
    private final List<Window> shorterWindowsCache = new ArrayList<>();
    private final List<Pair<List<Integer>, Window>> stack = new ArrayList<>();
    private final List<List<Integer>> occurrences = new ArrayList<>();
    private final Map<Integer, Integer> itemAtT = new HashMap<Integer, Integer>();
     ...
     * @param minsup             for filtering infrequent items, e.g. if minsup=2, this will remove item occurring less than 2 times in the entire
     *                           sequence
     * @param alpha              size of window, relative to pattern. e.g. alpha=2 will only count occurrences of (a,b,c) within a window of size
     *                           3*2=6
    ...
    */
    ...
    private int computeNumberOfMinimalWindowsBasedOnProjection(SequentialPattern X, Set<Integer> XNoneOverlapping, List<Window> projectionX,
                                                               int lengthZMax) {
