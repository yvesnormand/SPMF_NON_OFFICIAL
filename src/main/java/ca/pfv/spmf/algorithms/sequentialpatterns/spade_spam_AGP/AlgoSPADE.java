...
public class AlgoSPADE {
     ...
     * @throws IOException
     */
    public void runAlgorithm(SequenceDatabase database, CandidateGenerator candidateGenerator, boolean keepPatterns, boolean verbose,
                             String outputFilePath, boolean outputSequenceIdentifiers)
            throws IOException {
     ...
     * @throws IOException
     */
    public void runAlgorithmParallelized(SequenceDatabase database, CandidateGenerator candidateGenerator, boolean keepPatterns, boolean verbose,
                                         String outputFilePath, boolean outputSequenceIdentifiers)
            throws IOException {
        //If we do no have any file path
        if (outputFilePath == null) {
     ...
     * @param verbose            Flag for debugging purposes
     */
    protected void runSPADE(SequenceDatabase database, CandidateGenerator candidateGenerator, long minSupportCount, boolean dfs,
     ...
     * @param verbose            Flag for debugging purposes
     */
    protected void runSPADEFromSize2Sequences(SequenceDatabase database, CandidateGenerator candidateGenerator, long minSupportCount, boolean dfs,
                                              boolean keepPatterns, boolean verbose) {
        frequentItems = database.frequentItems();
    ...
    public String printStatistics() {
        final String sb = "=============  Algorithm - STATISTICS =============\n Total time ~ " +
                          getRunningTime() +
                          " ms\n" +
     ...
     * @param verbose            Flag for debugging purposes
     */
    protected void runSPADEFromSize2PatternsParallelized(SequenceDatabase database, CandidateGenerator candidateGenerator, long minSupportCount,
                                                         boolean dfs, boolean keepPatterns, boolean verbose) {
        ...
        while (frequentItems.size() > 0) {
            ...
            Callable<Void> callable = new FrequentPatternEnumerationFacade(frequentPatternEnumeration, frequentItem, dfs, keepPatterns, verbose,
                    saver);
     ...
     * @param verbose            Flag for debugging purposes
     */
    protected void runSPADEFromSize2PatternsParallelized2(SequenceDatabase database, CandidateGenerator candidateGenerator, long minSupportCount,
                                                          boolean dfs, boolean keepPatterns, boolean verbose) {
        frequentItems = database.frequentItems();
        Collection<Pattern> size1Sequences = getPatterns(frequentItems);
        ...
        while (frequentItems.size() > 0) {
            ...
            Callable<Void> callable = new FrequentPatternEnumerationFacade(frequentPatternEnumeration, frequentAtom, dfs, keepPatterns, verbose,
                    saver);
            Future<Void> future = pool.submit(callable);
            set.add(future);
            frequentItems.remove(frequentItems.size() - 1);
