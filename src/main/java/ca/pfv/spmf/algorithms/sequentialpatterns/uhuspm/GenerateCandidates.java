...
public class GenerateCandidates {
     ...
     */
    static List<SequenceList> generateNextGenerationCandidates(List<SequenceList> generation, int minUtility, float minProbability,
    ...
    private static SequenceList combine(SequenceList pattern1, SequenceList pattern2, int minUtility) {
        ...
        if (!S1.equals(S2)) {
            ...
            if (pattern2.itemsets.get(pattern2.itemsets.size() - 1).size() >= 2) {
                ...
                if (!EUCPProperty(combiner.itemsets, minUtility)) {
                    return null;
                }
