            ...
            for (int ti = 0; ti < t.length; ti++) {
                for (int ci = 0; ci < c.length; ci++) {
                    for (int ni = 0; ni < n.length; ni++) {
                        ...
                        out.write("~/data_generator/bin/seq_data_generator seq -ncust " + d[di] + " -slen " + c[ci] + " -tlen " + t[ti] + " -nitems" +
                                  " " + n[ni] + " -seq.npats 2000 -lit.npats 5000 -seq.patlen " + s + " -lit.patlen " + i + " -fname " + datasetName + "\n");
                    }
                }
            }
        }
