...
//			}
    ...
    public static void opus(CoverIsQ coverIsQ, int maxItemCount) {
        ...
        for (i = 0; i < coverIsQ.q.size(); i++) {
            ...
            final int newMaxItemCount = Math.max(maxItemCount, Global.tids.get(item).size());
            ...
            final float ubval = Global.searchByLift
                                ? ((count == 0) ? 0.0f : (1.0f / Utils.countToSup(maxItemCount)))
