...
public class AntColony {
     ...
     */
    private final HashMap<BitSet, Integer> visitedItemset = new HashMap();
    /**
     * The ant colony size
     */
...
//					}
                ...
                if (initPheromoneTimes == searchTimesIndex) {
                    searchMethod = Math.random() * this.searchMethodThreshold1;
                } else {
                    searchMethod = (searchTimesIndex == 0) ? (Math.random() * this.searchMethodThreshold1)
                                                           : Math.random();// the ant chosse a search method
                }
...
//                    System.out.println(this.getAnt(antIndex).getBitSet().cardinality()+"this.getAnt(antIndex)"+this.getAnt(antIndex).getPassFood
//                    ()+"    "+this.getAnt(antIndex).getCurrentFood());
                this.getAnt(antIndex).getBitSet().clear(toFood);
                this.getAnt(antIndex).setCurrentFood(this.getAnt(antIndex).getPassFood());
