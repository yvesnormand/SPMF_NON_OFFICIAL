...
public class AlgoTHUI {
    ...
    public void writeResultTofileUnord() throws IOException {
        ...
        while (iter.hasNext()) {
            ...
            PatternTHUI pattern = iter.next();
            final String buffer = pattern.prefix +
                                  // write separator
                                  " #UTIL: " +
                                  // write support
                                  pattern.utility;
    ...
    private int comparePatterns(PatternTHUI item1, PatternTHUI item2) {
        ...
        int i1 = Integer.parseInt(item1.prefix.split(" ")[0]);
        int i2 = Integer.parseInt(item2.prefix.split(" ")[0]);

        int compare = mapItemToTWU.get(i1) - mapItemToTWU.get(i2);
        return compare;
    }
