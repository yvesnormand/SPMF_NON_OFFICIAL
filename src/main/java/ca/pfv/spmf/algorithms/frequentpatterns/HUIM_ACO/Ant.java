...
public class Ant implements Comparable<Ant> {
    ...
    private static final List<Integer> existPheromones = new ArrayList<Integer>();
    private static final List<Double> decisionTable = new ArrayList<Double>();
    int otherToFood = 0;
    ...
    public void setExistedFoods(int index) {
        if (!this.existedFoods.contains(index)) {
            ...
            if (!this.noExistedFoods.contains(index)) {
                System.out.println("index" + index);
            }

            this.noExistedFoods.remove((Integer) index);
        }
    }
    ...
    public int searchFood(List<Integer> HTWUs_1, HUIS huis, Roulette roulette) {
        ...
        do {
            if (this.getNoExistedFoods().size() != 0) {
                ...
                } while (!this.getNoExistedFoods().contains(toItem));
        ...
        for (int i = 0; i < decisionTable.size(); i++) {
            if (i == 0) {
                if ((randNum >= 0) && randNum < decisionTable.get(0)) {
                    selectedPheromones = existPheromones.get(0);
                    break;
                }
            } else if ((randNum > decisionTable.get(i - 1)) && randNum <= decisionTable.get(i)) {
                selectedPheromones = existPheromones.get(i);
                break;
            }
        }
