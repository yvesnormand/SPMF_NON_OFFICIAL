...
public class AlgoMLHUIMiner {
    ...
    private final int[] itemsetBuffer = null;
    ...
    public void runAlgorithm(String inputTransactions, String inputTaxonomy, String output, double min_utility) throws IOException {
        ...
        try {
            ...
            while ((thisLineTaxonomy = myInputTaxnomy.readLine()) != null) {
                ...
                if (thisLineTaxonomy.isEmpty() || thisLineTaxonomy.charAt(0) == '#' || thisLineTaxonomy.charAt(0) == '@') {
                    continue;
                }
        ...
        try {
            ...
            while ((thisLineTransaction = myInputTransaction.readLine()) != null) {
                ...
                // if the line is a comment, is empty or is a kind of metadata
                if (thisLineTransaction.isEmpty() || thisLineTransaction.charAt(0) == '#' || thisLineTransaction.charAt(0) == '@') {
                    continue;
                }
        ...
        try {
            ...
            while ((thisLineTransaction = myInputTransaction.readLine()) != null) {
                ...
                // if the line is a comment, is empty or is a kind of metadata
                if (thisLineTransaction.isEmpty() || thisLineTransaction.charAt(0) == '#' || thisLineTransaction.charAt(0) == '%' || thisLineTransaction.charAt(0) == '@') {
                    continue;
                }
...
//						UtilityListMLHUIMiner taxUtilityListMLHUIMinerOfItemCopy = taxUtilityListCMLHUIMinerOfItem;
                    ...
                    for (int k = 0; k < listAncestorOfItem.size(); k++) {
                        ...
                        if (!ancestor.containsKey(listAncestorOfItem.get(k))) {
                            ...
                            // create the tuple
                            ElementMLHUIMiner parentElementMLHUIMiner = new ElementMLHUIMiner(tid, pair.utility,
                                    firstChildItemRemainingUtility.get(listAncestorOfItem.get(k)));
                            ...
                            //ElementMLHUIMiner parentElementMLHUIMiner = new ElementMLHUIMiner(tid, ancestor.get(listAncestorOfItem.get(k))+pair
                            // .utility, remainingUtility);
