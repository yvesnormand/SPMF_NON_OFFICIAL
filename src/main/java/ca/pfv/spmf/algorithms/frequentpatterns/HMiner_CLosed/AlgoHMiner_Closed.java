...
//                        utilityOfJumpingCLosure += (MCULs.get(j).sumNu + MCULs.get(j).sumCu);
                    ...
                    if (utilityOfJumpingCLosure >= minUtility) {
                        jumpnum1++;
                        CHUIs.addItemset(new Itemset(Xy, utilityOfJumpingCLosure, UL_X.getSupport()), Xy.length);
...
//                            utilityOfJumpingCLosure += MCULs.get(j).sumNu + MCULs.get(j).sumCu;
                        ...
                        if (count == 0 && UL_X.sumNu + UL_X.sumCu >= minUtility) {//count == 0 说明不存在前向扩展
                            nojumpnum++;
                            CHUIs.addItemset(new Itemset(X, UL_X.sumNu + UL_X.sumCu, UL_X.getSupport()), X.length);
