...
public class AlgoNAFCP {
    ...
    boolean subsumptionCheck(FCI f) {
        List<Integer> arr = hashFCIs.get(f.frequency);
        if (arr != null) {

            for (int i = 0; i < arr.size(); i++) {
                if (subsetCheck(f.items, fcis.get(arr.get(i)).items)) {
                    return true;
                }
            }
        }
    ...
    void findFCIs(List<FCI> Is, int level) throws IOException {
        for (int i = Is.size() - 1; i >= 0; i--) {
            ...
            for (int j = i - 1; j >= 0; j--) {
                ...
                FCI IsJ = Is.get(j);
                if (N_list_check(IsJ.nCs, IsI.nCs)) {
            ...
            if (!subsumptionCheck(IsI)) {
                ...
                if (hashFCIs.get(IsI.frequency) == null) {
                    ...
                    List<Integer> ar = hashFCIs.get(IsI.frequency);
                    ar.add(fcis.size() - 1);
                    hashFCIs.put(IsI.frequency, ar);
                }
            }
