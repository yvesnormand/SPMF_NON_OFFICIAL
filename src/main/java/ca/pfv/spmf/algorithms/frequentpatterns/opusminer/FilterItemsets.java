...
//		}
            ...
            while (subset_it.hasNext()) {
                ...
                while (j < i) {
                    j++;
                    itemsetRec currentSupset = supset_it.next();
    ...
    public static boolean checkSS2(ArrayList<tidset> uniqueTids, int no,
                                   ...
                                   double alpha) {
        if (no == 0) {
            return !(FisherTest.fisherTest(availabletids - tidsleft.size()
        ...
        return checkSS2(uniqueTids, no - 1, tidsleft, newtids, availabletids,
                count, alpha);
    }
...
//	    TID *ut_end = std::set_difference(tids[*it].begin(), tids[*it].end(), supsettids.begin(), supsettids.end(), &uniqueTids[i][0]);
        ...
        if (result) {
            // set up a process that will check whether uniqueCov.size() is significantly greater than can be predicted by assuming independence
            // between any partition of is
            ...
            for (int i = uniqueTids.size() - 2; i >= 0; i--) {
                result = checkSS2(uniqueTids, i, uniqueTids.get(i), tidsright, Global.noOfTransactions - supsettids.size(), uniqueCov.size(),
                        Global.getAlpha(is.size()));
