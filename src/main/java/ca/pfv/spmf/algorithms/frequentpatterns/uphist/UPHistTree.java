...
public class UPHistTree {
    ...
    public void addLocalTransaction(List<UPHistNode> localPath, int pathUtility,
        ...
        for (i = 0; i < size; i++) {
            ...
            if (child == null) {
                ...
                currentlocalNode = insertNewNode(currentlocalNode, item, nodeUtility, pathCount, false, (short) 0, localPath.get(i).histogram,
                        localPath.get(i).uid);
            } else {
                // there is a node already, we update it
                int currentNU = child.nodeUtility; // current node utility
     ...
     * @return the new node
     */
    private UPHistNode insertNewNode(UPHistNode currentlocalNode, int item, int nodeUtility, int pathCount, boolean global, int quantity,
                                     Hist hist_previous, UUID uid) {
//		UPHistNode child=currentlocalNode.getChildWithID(item);
