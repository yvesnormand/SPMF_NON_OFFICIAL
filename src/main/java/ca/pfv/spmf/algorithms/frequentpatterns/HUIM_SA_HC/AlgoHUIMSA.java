...
//				}
    ...
    public boolean pev_Check(ChroNode tempBAIndividual, List<Integer> list) {
        List<Integer> templist = new ArrayList<Integer>();// é惦ééééé0ééλéé
        ...
        midBitSet = (BitSet) tempBitSet.clone();// éé¼éмééé

        // itemééλͼéééééééééééééééʹééitemsetéééééééitemééééééééitemsetééȥééééitem
    ...
    class ChroNode implements Comparable {
        ...
        public void calculateFitness(int k, List<Integer> templist) {
            ...
            int sum, fitness = 0;
            for (m = 0; m < templist.size(); m++) { // mééééééé񼯺é
                p = templist.get(m).intValue(); // éépéééédatabaseéеétransaction
                i = 0;
                j = 0;
                q = 0;
                temp = 0;
