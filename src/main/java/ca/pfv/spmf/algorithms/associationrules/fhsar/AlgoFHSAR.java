    ...
    public void runAlgorithm(String input, String inputSAR, String output,
        ...
        while (((line = reader.readLine()) != null)) {
            ...
            for (Rule rule : sensitiveRules) {
                // the number of items from the antecedent- Váº¾ TRÃéI of the
                ...
                for (int i = 0; i < lineSplited.length; i++) {
                    ...
                    if (matchLeft.size() == rule.leftSide.size()
                        ...
                        break; // stop the loop because we know that this
                        // rule match already!
                    }
                }
            }
            ...
            while (!check) {
                ...
                for (Rule checkRule : checkRules) {
                    if (!(tdd.items.containsAll(checkRule.leftSide) && tdd.items
                            .containsAll(checkRule.rightSide))) {
                ...
                for (Integer selectedItem : tdd.setItemRank) {
                    ...
                    for (Rule checkingRule : checkingRules) {
                        ...
                        if (checkingRule.leftSide.contains(selectedItem)) {
                            break;
                        } else {
                            dem++;
                        }
