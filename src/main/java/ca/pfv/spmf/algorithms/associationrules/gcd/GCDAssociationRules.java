...
public class GCDAssociationRules {
    ...
    private final double inputConfThreshold;
    private final double inputFreqThreshold;
    ...
    private List<String> constructCSVLines(List<AssociationRule> associationRules) {
        ...
        for (AssociationRule rule : associationRules) {
            final String line = getOriginalFactorsForGCD(rule.getAntecedent()) + " ==> " +
                                getOriginalFactorsForGCD(rule.getConsequent()) + " #SUP: " +
                                rule.getSupport() + " #CONF: " +
                                rule.getConfidence();
            lines.add(line);
        }
