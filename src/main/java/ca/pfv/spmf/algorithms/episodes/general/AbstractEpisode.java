    ...
    public String toString() {
        ...
        for (int i = 0; i < episodeLength - 1; i++) {
            for (int j = 0; j < events.get(i).length - 1; j++) {
                returnString = returnString + events.get(i)[j] + " ";
            }
            returnString = returnString + events.get(i)[events.get(i).length - 1];
            returnString = returnString + " -1 ";
        }
        for (int j = 0; j < events.get(episodeLength - 1).length - 1; j++) {
            returnString = returnString + events.get(episodeLength - 1)[j] + " ";
        }
        returnString = returnString
                       + events.get(episodeLength - 1)[events.get(episodeLength - 1).length - 1];
        returnString = returnString + " -1 #SUP: " + this.support;
        return returnString;
    }
