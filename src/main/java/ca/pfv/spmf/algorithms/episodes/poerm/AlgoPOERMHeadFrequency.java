...
//					System.out.println("feweaw:" + anitKey.toString() + episode.toString());
                    ...
                    for (RuleInterval oneOccurRuleInter : oneOccurRuleInters) {
                        ...
                        int intervalEnd = Math.min(oneOccurRuleInter.antiStart + this.parameter.getWinlen(),
                                oneOccurRuleInter.start + this.parameter.getYSpan());
                        for (int i = intervalStart; i < intervalEnd; ++i) {
                            ...
                            if (i > oneOccurRuleInter.end) {
                                ...
                                ruleInteval = new RuleInterval(oneOccurRuleInter.antiStart, oneOccurRuleInter.antiEnd, oneOccurRuleInter.start,
                                        oneOccurRuleInter.end);
                            }
...
//			}
//			if (poerrule.getAntiEpisode().get(0) == 18 && poerrule.getConseEpisode().size() > 1 && poerrule.getConseEpisode().get(0) == 8 &&
//			poerrule.getConseEpisode().get(0) == 29) {
//				System.out.println("aaaa");
//			}
            String episodeRule = "";
