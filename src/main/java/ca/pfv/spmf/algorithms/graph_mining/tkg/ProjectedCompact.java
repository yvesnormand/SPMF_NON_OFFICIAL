...
public class ProjectedCompact {
    ...
    /**
     * computes MNI, minimum node image, defined by the minimum of |Number of graphs database vertices to which particular DFS code vertex is
     * projected|
     *
     * @return MNI value
     */
                ...
                for (int gid : index.keySet()) {
                    ...
                    for (int fromVertex : vertexEdges.keySet()) {
                        for (ProjectedEdge projectedEdge : vertexEdges.get(fromVertex)) {
                            int vertex = projectedEdge.isReversed()
    ...
    public void addProjection(ProjectedEdge projectedEdge) {
        ...
        if (!numProjectionsInGraph.containsKey(projectedEdge.getEdgeEnumeration().getGid())) {
            ...
            numProjectionsInGraph.put(projectedEdge.getEdgeEnumeration().getGid(),
                    numProjectionsInGraph.get(projectedEdge.getEdgeEnumeration().getGid()) + 1);
        }
    }
    ...
    public void addProjection(ProjectedEdge projectedEdge, int index, long projectionsNumber) {
        ...
        if (!numProjectionsInGraph.containsKey(projectedEdge.getEdgeEnumeration().getGid())) {
            ...
            numProjectionsInGraph.put(projectedEdge.getEdgeEnumeration().getGid(),
                    numProjectionsInGraph.get(projectedEdge.getEdgeEnumeration().getGid()) + projectionsNumber);
        }
    }
        ...
        for (Map<Integer, Set<ProjectedEdge>> vertexEdges : index.values()) {
            for (Set<ProjectedEdge> edges : vertexEdges.values()) {
                for (ProjectedEdge projectedEdge : edges) {
                    verticesEnumerations.add(new VertexEnumeration(projectedEdge.getEdgeEnumeration().getGid(), projectedEdge.isReversed()
                                                                                                                ?
                                                                                                                projectedEdge.getEdgeEnumeration().getEdge().v1
                                                                                                                :
                                                                                                                projectedEdge.getEdgeEnumeration().getEdge().v2));
