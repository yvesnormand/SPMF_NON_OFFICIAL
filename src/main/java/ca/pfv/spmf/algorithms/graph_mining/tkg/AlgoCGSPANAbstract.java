            ...
            for (Integer graphId : graphIds) {
                ...
                for (Vertex vertex : g.vertices) {
                    for (Edge e : vertex.getEdgeList()) {
                        ...
                        boolean isReversed = (v1L >= v2L && (v2L < v1L || vertex.getId() != e.v1));
...
//            	}
            ...
            ProjectedIteratorConsumer iterator = projected.iterator(ThreadPool.getProjectedIteratorProducersInstance().getThreadCount() * 10,
                    ThreadPool.getProjectedIteratorProducersInstance().getThreadCount(), callbacks);
            ...
            for (int i = 0; i < threadPool.getThreadCount(); i++) {
                RightPathExtenderSynchronized rightPathExtender = new RightPathExtenderSynchronized(extensionPDFSCounter, iterator, this, c,
                graphDatabase, extensions);
     ...
     */
    protected EarlyTerminationResult earlyTermination(Set<Integer> setOfGraphsIDs, ProjectedCompact projected,
                                                      IEarlyTerminationFailureHandler earlyTerminationFailureHandler) {
     ...
     */
    protected boolean checkEarlyTerminationFailure(ClosedSubgraph closedSubgraph, Map<Integer, Integer> isomorphism,
                                                   IEarlyTerminationFailureHandler earlyTerminationFailureHandler) {
     ...
     */
    protected void cgSpan(List<DatabaseGraph> graphDB, boolean outputClosedVertices, IEarlyTerminationFailureHandler earlyTerminationFailureHandler)
            throws IOException, ClassNotFoundException, InterruptedException {
     ...
     */
    protected abstract void outputClosedOneVertex(List<DatabaseGraph> graphDB, ProjectedCompact projected)
            throws IOException, ClassNotFoundException, InterruptedException;
     ...
     * @throws ClassNotFoundException if error casting a class
     */
    protected abstract void cgSpanDFS(DFSCode c, List<DatabaseGraph> graphDB, Set<Integer> graphIds, ProjectedCompact projected,
                                      IEarlyTerminationFailureHandler earlyTerminationFailureHandler)
            throws IOException, ClassNotFoundException, InterruptedException;
    ...
    protected class EarlyTerminationResult {
        ...
        private final boolean isEarlyTerminationFailure;
