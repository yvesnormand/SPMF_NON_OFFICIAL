...
public class IsomorphicSubgraph {
    ...
    private final DFSCode isomorphismDfsCode;
    // DFS code indices of isomorphism edges with identically labeled vertices indices in dfs code
    private final List<Integer> isomorphicEdgesIndices = new LinkedList<Integer>();
    // DFS code indices of edges that connect isomorphic edges
    private final List<Integer> internalNonIsomorphicEdgesIndices = new LinkedList<Integer>();
    // DFS code indices of edges which share one vertex with isomorphic edge
    private final List<Integer> outgoingEdgesIndices = new LinkedList<Integer>();
    // sorted DFS code indices of isomorphic, internal and outgoing edges
    private final List<Integer> edgesIndices = new LinkedList<Integer>();
    // vertices of isomorphic edges
    private final Set<Integer> isomorphicEdgesVertices = new HashSet<Integer>();
    // vertices of isomorphic edges that are on the rightmost path
    private final Set<Integer> isomorphismRightmostPathVertices = new HashSet<Integer>();
    // if rightmost vertex is included in isomorphic vertices
    private final boolean includesRightmostVertex;
     ...
     *                             2. projection in the set has same projected edges as canonical projection from DFS edges that do not belong to
     *                             the isomorphic subgraph.
     *                             3. projection in the set has same set of projected vertices as canonical projection from DFS code edges that
     *                             belong to the isomorphic subgraph.
     *                             For each projection in the isomorphic projections set, from the ordered list of isomorphic subgraph DFS code
     *                             isomorphic vertices construct list of their projections.
    ...
    public IsomorphicSubgraphProjections projections(ProjectedCompact projectedCompact, PDFSCompact pdfsCompactCanonical) {
        ...
        IsomorphicSubgraphProjections isomorphicSubgraphProjections = new IsomorphicSubgraphProjections(pdfsCompactCanonical.databaseGraph,
                pdfsCompactCanonical.projectedEdges, pdfsCompactCanonical.vertices, new HashSet<Integer>(edgesIndices));
        ...
        while (iterator.hasNext()) {
            ...
            for (int isomorphismRightmostPathVertex : isomorphismRightmostPathVertices) {
                ...
                isomorphicSubgraphProjections.addVertexRightMostPathIndexProjection(projectedIsomorphismRightmostPathVertex,
                        isomorphismRightmostPathVertex, projection);
            }
            ...
            if (includesRightmostVertex) {
                ...
                for (int isomorphismRightmostPathVertex : isomorphismRightmostPathVertices) {
                    ...
                    isomorphicSubgraphProjections.addRightMostVertexVertexRightMostPathIndexProjection(projectedIsomorphismRightmostVertex,
                            projectedIsomorphismRightmostPathVertex, isomorphismRightmostPathVertex, projection);
                }
            }
        }
     ...
     * For each projection in the isomorphic projections set, from the ordered list of isomorphic subgraph DFS code isomorphic vertices construct
     * list of their projections.
     * Canonical projection is the projection having the smallest list by the lexicographical order.
     */
                    ...
                    if (outgoingEdgesIndices.contains(i)) {
                        ...
                        if (isomorphicVertices.contains(outgoingEdge.v1)) {
                            for (int v1 : labelIsomorphicVerticesSorted.get(isomorphicVertexLabel)) {
                                ...
                                Edge edgeDB = new Edge(edge.v1 < edge.v2 ? edge.v1 : edge.v2, edge.v1 < edge.v2
                                                                                              ? edge.v2
                                                                                              : edge.v1, edge.getEdgeLabel());
                                EdgeEnumeration edgeEnumeration = new EdgeEnumeration(gid, edgeDB);
                        ...
                        int v1 = pdfs.get(i).isReversed()
                                 ? pdfs.get(i).getEdgeEnumeration().getEdge().v2
                                 : pdfs.get(i).getEdgeEnumeration().getEdge().v1;
                        if (!projectionsAt.get(gid).containsKey(v1)) {
                            projectionsAt.get(gid).put(v1, new LinkedHashSet<ProjectedEdge>());
                        }
