...
public class RightPathExtender implements Callable<Integer> {
    ...
    private final Map<ExtendedEdge, Integer> extensionPDFSCounter;
    // projections iterator
    private final ProjectedIteratorConsumer iterator;
    // activation class
    private final AlgoCGSPANAbstract algoCGSPANAbstractCompact;
    // DFS code to be right extended
    private final DFSCode c;
    // graphs database
    private final List<DatabaseGraph> graphDatabase;
    // global right extensions
    private final Map<ExtendedEdge, ProjectedCompact> extensions;
    // right extensions produced by this thread
    private final Map<ExtendedEdge, ProjectedCompact> extensionsLocal;
    // number of projections that could be extended with extended edge by this thread
    private final Map<ExtendedEdge, Integer> extensionPDFSCounterLocal;

    public RightPathExtender(Map<ExtendedEdge, Integer> extensionPDFSCounter, ProjectedIteratorConsumer iterator,
                             AlgoCGSPANAbstract algoCGSPANAbstractCompact, DFSCode c, List<DatabaseGraph> graphDatabase, Map<ExtendedEdge,
            ProjectedCompact> extensions) {
    ...
    public Integer call() throws Exception {
        ...
        while (true) {
            ...
            if (AlgoCGSPANAbstract.EDGE_COUNT_PRUNING && c.size() >= g.getEdgeCount()) {
    ...
    protected void rightMostPathBackwardExtensions(DFSCode c, int rightMost, List<DatabaseGraph> graphDatabase, DatabaseGraph g, PDFSCompact pdfs,
                                                   Map<Integer, Integer> isom)
    ...
    protected void rightMostPathBackwardExtensionsAutomorphism(DFSCode c, int rightMost, List<DatabaseGraph> graphDatabase, DatabaseGraph g,
                                                               PDFSCompact pdfs, Map<Integer, Integer> isom)
            throws IOException, ClassNotFoundException {
                ...
                if (!projections.getVertexRightMostPathIndexProjections().containsKey(x.getId())) {
                    ...
                    for (int invertedX : projections.getVertexRightMostPathIndexProjections().get(x.getId()).keySet()) {
                        if (c.onRightMostPath(invertedX) && c.notPreOfRM(invertedX)
                            ...
                            extensionPDFSCounterLocal.put(ee,
                                    extensionPDFSCounterLocal.get(ee) + projections.getVertexRightMostPathIndexProjectionsCount().get(x.getId()).get(invertedX));

                            Edge e = g.getEdge(mappedRM, x.getId());
                            EdgeEnumeration edgeEnumeration = g.getEdgeEnumeration(e);
                            boolean isReversed = e.v1 != mappedRM;
                            ProjectedEdge projectedEdge = ProjectedEdge.get(edgeEnumeration, isReversed);
                            extensionProjected.addProjection(projections.getVertexRightMostPathIndexProjections().get(x.getId()).get(invertedX),
                                    projections.getVertexRightMostPathIndexProjectionsCount().get(x.getId()).get(invertedX), projectedEdge);
                        }
                    }
                }
            }
                ...
                for (Vertex x : g.getAllNeighbors(mappedRM)) {
                    ...
                    if (!projections.getRightMostVertexVertexRightMostPathIndexProjections().get(mappedRM).containsKey(x.getId())) {
                        ...
                        if (invertedX != null && c.onRightMostPath(invertedX) && c.notPreOfRM(invertedX)
                            ...
                            extensionPDFSCounterLocal.put(ee,
                                    extensionPDFSCounterLocal.get(ee) + projections.getVertexRightMostPathIndexProjectionsCount().get(mappedRM).get(rightMost));

                            Edge e = g.getEdge(mappedRM, x.getId());
                            EdgeEnumeration edgeEnumeration = g.getEdgeEnumeration(e);
                            boolean isReversed = e.v1 != mappedRM;
                            ProjectedEdge projectedEdge = ProjectedEdge.get(edgeEnumeration, isReversed);
                            extensionProjected.addProjection(projections.getVertexRightMostPathIndexProjections().get(mappedRM).get(rightMost),
                                    projections.getVertexRightMostPathIndexProjectionsCount().get(mappedRM).get(rightMost), projectedEdge);
                        }
                    } else {
                        // 'to' vertex of the backward edge belongs to isomorphic subgraph on the projection
                        for (int invertedX :
                                projections.getRightMostVertexVertexRightMostPathIndexProjections().get(mappedRM).get(x.getId()).keySet()) {
                            if (c.onRightMostPath(invertedX) && c.notPreOfRM(invertedX)
                                && !c.containEdge(rightMost, invertedX)) {
                                // rightmost and invertedX both have correspondings in g, so label of vertices
                                // and edge all
                                ...
                                extensionPDFSCounterLocal.put(ee,
                                        extensionPDFSCounterLocal.get(ee) + projections.getRightMostVertexVertexRightMostPathIndexProjectionsCount().get(mappedRM).get(x.getId()).get(invertedX));
    ...
    protected void rightMostPathForwardExtensions(DFSCode c, int rightMost, List<DatabaseGraph> graphDatabase, DatabaseGraph g, PDFSCompact pdfs,
                                                  Map<Integer, Integer> isom)
            throws IOException, ClassNotFoundException {
    ...
    private void addForwardExtensionProjection(DFSCode c, int v, int rightMost, List<DatabaseGraph> graphDatabase, DatabaseGraph g,
                                               PDFSCompact pdfs, int mappedV, int mappedVlabel, int xLabel, Edge e,
                                               Set<ExtendedEdge> countedExtensions)
            throws IOException, ClassNotFoundException {
    ...
    protected void rightMostPathForwardExtensionsAutomorphism(DFSCode c, int rightMost, List<DatabaseGraph> graphDatabase, DatabaseGraph g,
                                                              PDFSCompact pdfs, Map<Integer, Integer> isom)
            throws IOException, ClassNotFoundException {
        ...
        for (int v : c.getRightMostPath()) {
            ...
            for (Edge edge : g.vMap.get(mappedV).getEdgeList()) {
                ...
                int xLabel = g.vMap.get(x).getLabel();
                if (!mappedVertices.contains(x)) {
                    addForwardExtensionProjectionAutomorphism(c, v, rightMost, graphDatabase, g, projections.getProjections(),
                            projections.getNumProjections(), mappedV, mappedVlabel, xLabel, edge, countedExtensions);
                }
            }
        }
            ...
            for (Edge edge : g.vMap.get(mappedV).getEdgeList()) {
                ...
                if (!mappedVertices.contains(x)) {
                    for (int v : projections.getVertexRightMostPathIndexProjections().get(mappedV).keySet()) {
                        if (!countedExtensions.containsKey(v)) {
                            countedExtensions.put(v, new HashSet<ExtendedEdge>());
                        }
                        addForwardExtensionProjectionAutomorphism(c, v, rightMost, graphDatabase, g,
                                projections.getVertexRightMostPathIndexProjections().get(mappedV).get(v),
                                projections.getVertexRightMostPathIndexProjectionsCount().get(mappedV).get(v), mappedV, mappedVlabel, xLabel, edge,
                                countedExtensions.get(v));
                    }
                }
            }
        }
    ...
    private void addForwardExtensionProjectionAutomorphism(DFSCode c, int v, int rightMost, List<DatabaseGraph> graphDatabase, DatabaseGraph g,
                                                           List<Set<ProjectedEdge>> projections, int numProjections, int mappedV, int mappedVlabel,
                                                           int xLabel, Edge e, Set<ExtendedEdge> countedExtensions)
            throws IOException, ClassNotFoundException {
        ExtendedEdge ee = new ExtendedEdge(v, rightMost + 1, mappedVlabel, xLabel,
                e.getEdgeLabel());
