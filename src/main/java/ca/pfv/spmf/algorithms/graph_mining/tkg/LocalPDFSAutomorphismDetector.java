...
public class LocalPDFSAutomorphismDetector {
    ...
    private final ProjectedCompact projectedCompact;
    // index of isomorphic subgraph by the maximum index of the DFS code edge that belongs to the isomorphic subgraph
    private final Map<Integer, IsomorphicSubgraph> isomorphismsMaxIndices = new HashMap<Integer, IsomorphicSubgraph>();
    ...
    private void connectedComponents() {
        ...
        for (ExtendedEdge ee1 : isomorphicExtendedEdges.keySet()) {
            for (ExtendedEdge extendedEdge : dfsCode.getEeL()) {
                ...
                if (isomorphicExtendedEdges.containsKey(extendedEdge)) {
                    continue;
                }
    ...
    private void connectedComponentsDFS(ExtendedEdge extendedEdge, Map<ExtendedEdge, Set<ExtendedEdge>> isomorphicExtendedEdges, Map<ExtendedEdge,
            Boolean> visited, Map<ExtendedEdge, Integer> components, int component) {
    ...
    public PDFSCompact beforeSubmit(PDFSCompact pdfsCompactCanonical) {
        IsomorphicSubgraphProjections projections = new IsomorphicSubgraphProjections(pdfsCompactCanonical.databaseGraph,
                pdfsCompactCanonical.projectedEdges, pdfsCompactCanonical.vertices, new HashSet<Integer>());
