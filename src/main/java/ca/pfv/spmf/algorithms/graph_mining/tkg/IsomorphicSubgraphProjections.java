...
public class IsomorphicSubgraphProjections extends PDFSCompact {
    ...
    private Map<Integer, Map<Integer, List<Set<ProjectedEdge>>>> vertexRightMostPathIndexProjections = new HashMap<Integer, Map<Integer,
            List<Set<ProjectedEdge>>>>();
    ...
    private Map<Integer, Map<Integer, Map<Integer, List<Set<ProjectedEdge>>>>> rightMostVertexVertexRightMostPathIndexProjections =
            new HashMap<Integer, Map<Integer, Map<Integer, List<Set<ProjectedEdge>>>>>();
    // number of projections from rightmost vertex to vertex on a rightmost path index
    private Map<Integer, Map<Integer, Map<Integer, Integer>>> rightMostVertexVertexRightMostPathIndexProjectionsCount = new HashMap<Integer,
    ...
    public IsomorphicSubgraphProjections(DatabaseGraph databaseGraph, List<ProjectedEdge> projectedEdges, List<Vertex> vertices,
                                         Set<Integer> isomorphismEdgesIndices) {
    ...
    public void addVertexRightMostPathIndexProjection(int vertex, int rightMostPathIndex, List<ProjectedEdge> projection) {
        ...
        vertexRightMostPathIndexProjectionsCount.get(vertex).put(rightMostPathIndex,
                vertexRightMostPathIndexProjectionsCount.get(vertex).get(rightMostPathIndex) + 1);
    }
     ...
     * @param projection         projection to add
     */
    public void addRightMostVertexVertexRightMostPathIndexProjection(int rightMostVertex, int vertex, int rightMostPathIndex,
                                                                     List<ProjectedEdge> projection) {
        ...
        if (!rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).get(vertex).containsKey(rightMostPathIndex)) {
            rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).get(vertex).put(rightMostPathIndex,
                    new ArrayList<Set<ProjectedEdge>>());
        ...
        rightMostVertexVertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(vertex).put(rightMostPathIndex,
                rightMostVertexVertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(vertex).get(rightMostPathIndex) + 1);
    }
    ...
    public IsomorphicSubgraphProjections merge(IsomorphicSubgraphProjections other, int rightMostIndex) {
        ...
        for (int vertex : vertexRightMostPathIndexProjections.keySet()) {
            ...
            for (int rightMostPathIndex : vertexRightMostPathIndexProjections.get(vertex).keySet()) {
                ...
                if (other.vertexRightMostPathIndexProjections.containsKey(vertex) && other.vertexRightMostPathIndexProjections.get(vertex).containsKey(rightMostPathIndex)) {
                    merged.vertexRightMostPathIndexProjectionsCount.get(vertex).put(rightMostPathIndex,
                            vertexRightMostPathIndexProjectionsCount.get(vertex).get(rightMostPathIndex) * other.vertexRightMostPathIndexProjectionsCount.get(vertex).get(rightMostPathIndex));
                    for (int i = 0; i < projections.size(); i++) {
                    ...
                    merged.vertexRightMostPathIndexProjectionsCount.get(vertex).put(rightMostPathIndex,
                            vertexRightMostPathIndexProjectionsCount.get(vertex).get(rightMostPathIndex) * other.getNumProjections());
                    for (int i = 0; i < projections.size(); i++) {
                        Set<ProjectedEdge> mergedProjection = new HashSet<ProjectedEdge>();
                        if (isomorphismEdgesIndices.contains(i)) {
                            mergedProjection.addAll(vertexRightMostPathIndexProjections.get(vertex).get(rightMostPathIndex).get(i));
        ...
        for (int vertex : other.vertexRightMostPathIndexProjections.keySet()) {
            ...
            for (int rightMostPathIndex : other.vertexRightMostPathIndexProjections.get(vertex).keySet()) {
                if (merged.vertexRightMostPathIndexProjections.get(vertex).containsKey(rightMostPathIndex)) {
                    continue;
                }
                merged.vertexRightMostPathIndexProjections.get(vertex).put(rightMostPathIndex, new ArrayList<Set<ProjectedEdge>>());

                merged.vertexRightMostPathIndexProjectionsCount.get(vertex).put(rightMostPathIndex,
                        other.vertexRightMostPathIndexProjectionsCount.get(vertex).get(rightMostPathIndex) * getNumProjections());
                for (int i = 0; i < projections.size(); i++) {
                    Set<ProjectedEdge> mergedProjection = new HashSet<ProjectedEdge>();
                    if (isomorphismEdgesIndices.contains(i)) {
                        mergedProjection.addAll(projections.get(i));
        ...
        // merge projections which have particular rightmost vertex projection and also have particular database vertex projected from a rightmost
        // path index
        for (int rightMostVertex : rightMostVertexVertexRightMostPathIndexProjections.keySet()) {
            merged.rightMostVertexVertexRightMostPathIndexProjections.put(rightMostVertex, new HashMap<Integer, Map<Integer,
                    List<Set<ProjectedEdge>>>>());
            ...
            for (int vertex : rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).keySet()) {
                merged.rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).put(vertex, new HashMap<Integer,
                        List<Set<ProjectedEdge>>>());
                ...
                for (int rightMostPathIndex : rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).get(vertex).keySet()) {
                    merged.rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).get(vertex).put(rightMostPathIndex,
                            new ArrayList<Set<ProjectedEdge>>());
                    merged.rightMostVertexVertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(vertex).put(rightMostPathIndex,
                            rightMostVertexVertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(vertex).get(rightMostPathIndex) * other.getNumProjections());
                    for (int i = 0; i < projections.size(); i++) {
            ...
            for (int otherVertex : other.vertexRightMostPathIndexProjections.keySet()) {
                merged.rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).put(otherVertex, new HashMap<Integer,
                        List<Set<ProjectedEdge>>>());
                ...
                for (int otherIndex : other.vertexRightMostPathIndexProjections.get(otherVertex).keySet()) {
                    merged.rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).get(otherVertex).put(otherIndex,
                            new ArrayList<Set<ProjectedEdge>>());
                    merged.rightMostVertexVertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(otherVertex).put(otherIndex,
                            vertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(rightMostIndex) * other.vertexRightMostPathIndexProjectionsCount.get(otherVertex).get(otherIndex));
                    for (int i = 0; i < projections.size(); i++) {
                        Set<ProjectedEdge> mergedProjection = new HashSet<ProjectedEdge>();
        ...
        for (int rightMostVertex : other.rightMostVertexVertexRightMostPathIndexProjections.keySet()) {
            if (!merged.rightMostVertexVertexRightMostPathIndexProjections.containsKey(rightMostVertex)) {
                merged.rightMostVertexVertexRightMostPathIndexProjections.put(rightMostVertex, new HashMap<Integer, Map<Integer,
                        List<Set<ProjectedEdge>>>>());
                merged.rightMostVertexVertexRightMostPathIndexProjectionsCount.put(rightMostVertex, new HashMap<Integer, Map<Integer, Integer>>());
            }
            for (int vertex : other.rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).keySet()) {
                ...
                for (int rightMostPathIndex : other.rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).get(vertex).keySet()) {
                    merged.rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).get(vertex).put(rightMostPathIndex,
                            new ArrayList<Set<ProjectedEdge>>());
                    merged.rightMostVertexVertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(vertex).put(rightMostPathIndex,
                            other.rightMostVertexVertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(vertex).get(rightMostPathIndex) * getNumProjections());
                    for (int i = 0; i < projections.size(); i++) {
            ...
            for (int myVertex : vertexRightMostPathIndexProjections.keySet()) {
                ...
                merged.rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).put(myVertex, new HashMap<Integer,
                        List<Set<ProjectedEdge>>>());
                ...
                for (int myIndex : vertexRightMostPathIndexProjections.get(myVertex).keySet()) {
                    merged.rightMostVertexVertexRightMostPathIndexProjections.get(rightMostVertex).get(myVertex).put(myIndex,
                            new ArrayList<Set<ProjectedEdge>>());
                    merged.rightMostVertexVertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(myVertex).put(myIndex,
                            other.vertexRightMostPathIndexProjectionsCount.get(rightMostVertex).get(rightMostIndex) * vertexRightMostPathIndexProjectionsCount.get(myVertex).get(myIndex));
                    for (int i = 0; i < projections.size(); i++) {
