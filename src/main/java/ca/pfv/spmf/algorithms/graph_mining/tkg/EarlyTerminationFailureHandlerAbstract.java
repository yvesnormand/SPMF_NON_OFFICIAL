...
public abstract class EarlyTerminationFailureHandlerAbstract implements IEarlyTerminationFailureHandler {
     ...
     */
    protected abstract boolean analyzeCase1(DFSCode dfsCode, ProjectedCompact projected, List<Integer> rightMostPathEdges, Map<ExtendedEdge,
     ...
     * @return true if the check evaluates to true, false otherwise
     */
    protected abstract boolean analyzeCase2(DFSCode dfsCode, ProjectedCompact projected, List<Integer> rightMostPathEdges, Map<ExtendedEdge,
            ProjectedCompact> forwardExtensions);
     ...
     * @return true if the check evaluates to true, false otherwise
     */
    protected abstract boolean analyzeCase3(DFSCode dfsCode, ProjectedCompact projected, List<Integer> rightMostPathEdges, Map<ExtendedEdge,
            ProjectedCompact> forwardExtensions);
    ...
    protected boolean analyzeCase4(DFSCode dfsCode, ProjectedCompact projected, List<Integer> rightMostPathEdges) {
        ...
        if (dfsCode.getAt(length - 1).v2 < dfsCode.getAt(length - 1).v1) {
            ...
            ProjectedIteratorConsumer iterator = projected.iterator(ThreadPool.getProjectedIteratorProducersInstance().getThreadCount() * 2,
                    ThreadPool.getProjectedIteratorProducersInstance().getThreadCount());
    ...
    protected class TrieNode {
        private final Map<ExtendedEdge, TrieNode> children;
