...
public class ProjectedIteratorProducer implements Runnable {
    ...
    private final ProjectedIteratorConsumer.Control control;
    ...
    public ProjectedIteratorProducer(ProjectedCompact projected, BlockingQueue<PDFSCompact> pdfsQueue, ProjectedIteratorConsumer.Control control,
                                     List<ProjectedEdge> firstEdges) {
    ...
    public ProjectedIteratorProducer(ProjectedCompact projected, BlockingQueue<PDFSCompact> pdfsQueue, ProjectedIteratorConsumer.Control control,
                                     List<ProjectedEdge> firstEdges, List<IProjectedIteratorCallback> callbacks) {
        this.projected = projected;
        this.pdfsQueue = pdfsQueue;
        this.control = control;
    ...
    public ProjectedIteratorProducer(ProjectedCompact projected, BlockingQueue<PDFSCompact> pdfsQueue, ProjectedIteratorConsumer.Control control,
                                     List<ProjectedEdge> firstEdges, int gid) {
        this.projected = projected;
        this.pdfsQueue = pdfsQueue;
        this.control = control;
    ...
    public ProjectedIteratorProducer(ProjectedCompact projected, BlockingQueue<PDFSCompact> pdfsQueue, ProjectedIteratorConsumer.Control control,
                                     List<ProjectedEdge> firstEdges, int gid, List<IProjectedIteratorCallback> callbacks) {
        this.projected = projected;
        this.pdfsQueue = pdfsQueue;
        this.control = control;
        this.callbacks = callbacks;
            ...
            while (edgeIterator.hasNext()) {
                ...
                if (nextExtendedEdge.v2 > nextExtendedEdge.v1) {
                    ...
                    Iterator<ProjectedEdge> nextEdgeIterator =
                            projected.getProjected().get(pdfs.size()).get(databaseGraph.getId()).get(nextV.getId()).iterator();
