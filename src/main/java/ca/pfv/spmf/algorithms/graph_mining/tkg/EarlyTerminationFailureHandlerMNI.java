...
public class EarlyTerminationFailureHandlerMNI extends EarlyTerminationFailureHandlerAbstract {
     ...
     */
    protected boolean analyzeCase1(DFSCode dfsCode, ProjectedCompact projected, List<Integer> rightMostPathEdges, Map<ExtendedEdge,
            ProjectedCompact> forwardExtensions) {
        int length = projected.getProjected().size();

        if (dfsCode.getAt(length - 1).v2 > dfsCode.getAt(length - 1).v1) {
            Map<ElbVlbKey, Set<ProjectedCompact.VertexEnumeration>> elbVlbVertices = new HashMap<ElbVlbKey,
                    Set<ProjectedCompact.VertexEnumeration>>();
     ...
     * @return true if the check evaluates to true, false otherwise
     */
    protected boolean analyzeCase2(DFSCode dfsCode, ProjectedCompact projected, List<Integer> rightMostPathEdges, Map<ExtendedEdge,
            ProjectedCompact> forwardExtensions) {
        int length = projected.getProjected().size();

        if (dfsCode.getAt(length - 1).v2 > dfsCode.getAt(length - 1).v1) {
            Map<VertexElbVlbKey, Set<ProjectedCompact.VertexEnumeration>> vertexElbVlbVertices = new HashMap<VertexElbVlbKey,
                    Set<ProjectedCompact.VertexEnumeration>>();
     ...
     * @return true if the check evaluates to true, false otherwise
     */
    protected boolean analyzeCase3(DFSCode dfsCode, ProjectedCompact projected, List<Integer> rightMostPathEdges, Map<ExtendedEdge,
            ProjectedCompact> forwardExtensions) {
        int length = projected.getProjected().size();
