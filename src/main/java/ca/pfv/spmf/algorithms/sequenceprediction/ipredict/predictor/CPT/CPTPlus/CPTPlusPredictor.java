 ...
 * <p>
 * Source: T. Gueniche, P. Fournier-Viger and V. S. Tseng, "CPT+: Decreasing the time/space complexity of the Compact Prediction Tree" In the
...
public class CPTPlusPredictor extends Predictor implements Serializable {
     ...
     */
    private final boolean CCF = false;
    /**
     * Flag for the CBS Strategy (default value)
     */
    private final boolean CBS = true;
    ...
    public Boolean Train(List<Sequence> trainingSequences) {
        ...
        if (parameters.paramBoolOrDefault("CCF", CCF)) {
            List<List<Item>> itemsets = finder.findFrequentItemsets(trainingSequences, parameters.paramInt("CCFmin"), parameters.paramInt("CCFmax")
                    , parameters.paramInt("CCFsup"));
    ...
    protected CountTable predictionByActiveNoiseReduction(Sequence target) {
        ...
        //Setting parameters
        int maxPredictionCount = 1 + (int) (target.size() * parameters.paramDouble("minPredictionRatio")); //minimum number of required prediction
        // to ensure the best accuracy
        int predictionCount = 0; //current number of prediction done (one by default because of the CountTable being updated with the target
        // initially)
