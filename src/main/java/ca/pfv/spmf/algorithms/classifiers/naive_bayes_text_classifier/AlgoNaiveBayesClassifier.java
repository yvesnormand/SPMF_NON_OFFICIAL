...
public class AlgoNaiveBayesClassifier {
    private final HashMap<String, List<File>> mFileLists = new HashMap<String, List<File>>();
    private final ArrayList<MemoryFile> mMemFiles = new ArrayList<MemoryFile>();
    ...
    private void runAlgorithm() throws Exception {
        ...
        ArrayList<OccurrenceProbabilties> op = new ArrayList<OccurrenceProbabilties>(); //A cache to hold probability for already calculated words
        // in each class
    ...
    private double calculateProbabilityInMemory(String word, ArrayList<OccurrenceProbabilties> op, String currentClass) {
        ...
        for (MemoryFile memFile : mMemFiles) {
            if (memFile.getClassname().equals(currentClass)) {
                occurances += Collections.frequency(memFile.getContent(), word) * 50; //Giving more weightage to occurances rather than just
                // incrementing by 1.
                count += memFile.getContent().size();
            }
        }
        prob = ((double) occurances + 50.0) / ((double) count + 100.0); // Normalizing the value to avoid return of 0.0
        return prob;
    }
