...
class SupportCounting {
     ...
     */
    private final Map<Item, Set<Pattern>> indexationMap;
            ...
            for (Pattern candidate : candidateSet) {
                ...
                if (finder.isPresent()) {
                    ...
                    for (int j = 0; j < position.size(); j++) {
                        str_primaryitemsettime =
                                Integer.toString((int) (database.getSequences().get(sequence.getId()).get(position.get(j)[0]).getTimestamp()));
        ...
        for (Pattern candidate : candidateSet) {
            ...
            for (int i = 0; i < candidate.getAppearingIn().length(); i++) {
                if (candidate.getAppearingIn().get(i)) {
                    each_length_constraint_sup =
                            Math.exp((-1.0 * k1) * (1.0 / max_sequence_length) * SequenceDatabase.primarysequences.get(i).getLength());
