...
class RecursionSPM_FC_P {
    ...
    public RecursionSPM_FC_P(AbstractionCreator abstractionCreator, Saver saver, long minSupportAbsolute,
                             PseudoSequenceDatabase pseudoDatabase, Map<Item, BitSet> mapSequenceID, SequenceDatabase originalDataset,
                             double support, double alpha2, double beta2, double gamma) {
        ...
        for (Item item : keySetList) {
            ...
            for (int m = prefix.getAppearingIn().nextSetBit(0); m >= 0; m = prefix.getAppearingIn().nextSetBit(m + 1)) {
                ...
                each_length_constraint_sup = Math
                        .exp((-1.0) * (k1 / max_sequence_length) * originalDataset.SItemTime.get(m - 1).size());
            ...
            if (projectedContext != null && projectedContext.size() > minSupportAbsolute) { // 鎴戝幓鎺変簡绛変簬鍙é
